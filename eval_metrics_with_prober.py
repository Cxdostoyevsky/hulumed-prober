...
def discover_result_files(input_dir: str) -> List[str]:
    ...
    for name in os.listdir(input_dir):
        if not name.endswith("mlp4_results.json"):
            continue
        # 兼容 probe_model_ 前缀和误写的 robe_model_ 前缀
        if name.startswith("probe_model_"):
...
def main():
    args = parse_args()
    output_fig = args.output_fig or os.path.join(args.input_dir, "probe_accuracy_4.png")
    output_json = args.output_json or os.path.join(args.input_dir, "probe_accuracy_summary_4.json")
