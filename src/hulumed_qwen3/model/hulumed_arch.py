...
class HulumedMetaModel:
    ...
    def __init__(self, config):
        ...
        if has_vision_encoder:
            ...
            # 变为            model.vision_encoder.encoder...（与官方一致）
        ...
        probe_position = getattr(config, 'probe_position', None)
        if probe_position is not None:
            probe_input_dim = self._get_probe_input_dim(config, probe_position)
            self.probe = ProbeHead(
                input_dim=probe_input_dim,
                num_classes=3,
                probe_type=getattr(config, 'probe_type', 'mlp'),
                hidden_dim=getattr(config, 'probe_hidden_dim', None),
            )
        else:
            self.probe = None
    ...
    def _get_probe_input_dim(self, config, probe_position: str) -> int:
        """根据 probe_position 返回探针输入维度"""
        if probe_position.startswith("vision_encoder_"):
            ve = self.get_vision_encoder()
            return getattr(config, 'mm_hidden_size', 768)
        if probe_position == "projector":
            return config.hidden_size
        if probe_position.startswith("llm_"):
            return config.hidden_size
        raise ValueError(f"Unknown probe_position: {probe_position}")

    def _parse_probe_position(self, probe_position: str) -> Tuple[str, Optional[int]]:
        """解析 probe_position 为 (stage, layer_idx)"""
        if probe_position.startswith("vision_encoder_"):
            layer_idx = int(probe_position.split("_")[-1])
            return "vision_encoder", layer_idx
        if probe_position == "projector":
            return "projector", None
        if probe_position.startswith("llm_"):
            layer_idx = int(probe_position.split("_")[-1])
            return "llm", layer_idx
        raise ValueError(f"Unknown probe_position: {probe_position}")
...
class HulumedMetaForCausalLM(ABC):
    ...
    def _per_image_mean_pool(
        self,
        features: torch.FloatTensor,
        split_sizes: List[int],
    ) -> torch.FloatTensor:
        """按样本切分后做 mean pooling，返回 [B, D]"""
        if sum(split_sizes) == 0:
            return features
        chunks = features.split(split_sizes, dim=0)
        return torch.stack([c.mean(dim=0) for c in chunks if c.shape[0] > 0], dim=0)

    def encode_for_probe(
        self,
        probe_position: str,
        pixel_values: torch.FloatTensor,
        grid_sizes: torch.LongTensor,
        ...
        """
        获取探针位置的特征，返回 (pooled_features [B,D], batched_num_patches 或 None)。
        对于 llm 位置，batched_num_patches 为 None，pooled_features 已按序列 mean pool。
        """
        model = self.get_model()
        stage, layer_idx = model._parse_probe_position(probe_position)
        ...
        if stage == "vision_encoder":
            mm_features = self.encode_images(
                pixel_values, grid_sizes, merge_sizes, stop_at_layer=layer_idx
            )
            split_sizes = grid_sizes.prod(dim=1).tolist()
            batched_num_patches = torch.tensor(
                split_sizes, device=mm_features.device, dtype=torch.long
            )
            mm_features = self._get_valid_visual_tokens(
                mm_features, batched_num_patches, modals
            )
            valid_sizes = [int(n) for n, m in zip(split_sizes, modals) if m != "text"]
            pooled = self._per_image_mean_pool(mm_features, valid_sizes)
            return pooled, batched_num_patches
        ...
        if stage == "projector":
            ...
            valid_sizes = [int(n) for n, m in zip(batched_num_patches.tolist(), modals) if m != "text"]
            pooled = self._per_image_mean_pool(mm_features, valid_sizes)
            return pooled, batched_num_patches

        if stage == "llm":
            # 改用完整 forward + hook 提取指定层特征，避免逐层调用时 position_embeddings 等参数缺失
            (
                _,
                _,
                _,
            ...
            if inputs_embeds is None:
                raise ValueError("prepare_inputs_labels_for_multimodal returned None inputs_embeds")
            # 兼容 PEFT 包装：layers 可能在 model.layers / model.model.layers / model.model.model.layers
            layers = None
            for obj in (model, getattr(model, "model", None), getattr(getattr(model, "model", None), "model", None)):
                if obj is not None:
                    ...
                    if layers is not None:
                        break
            if layers is None:
                raise ValueError("Cannot find model.layers for LLM probe")
            if layer_idx < 0 or layer_idx >= len(layers):
                raise ValueError(f"probe_position llm_{layer_idx} out of range (0~{len(layers)-1})")
            # 用 hook 捕获第 layer_idx 层的输出（仅训练探针时模型冻结，无需梯度回传）
            captured = [None]
            ...
            if captured[0] is None:
                raise RuntimeError(f"Hook did not capture output for llm layer {layer_idx}")
            
            # 只对图像特征部分做平均
            # captured[0]: [B, seq_len, D], image_mask: [B, seq_len]
            image_mask = image_mask.to(captured[0].device, dtype=captured[0].dtype)
            image_features = captured[0] * image_mask.unsqueeze(-1)
            pooled = image_features.sum(dim=1) / image_mask.sum(dim=1, keepdim=True).clamp(min=1)
            return pooled, None
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # 训练时 DataCollator 通常不传 modals，默认全为 image
        if modals is None:
